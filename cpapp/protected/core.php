...
defined('DEFAULT_APP_NAME') or define('DEFAULT_APP_NAME', 'default');
defined('DEFAULT_CONTROLLER_NAME') or define('DEFAULT_CONTROLLER_NAME', 'index');
defined('DEFAULT_ACTION_NAME') or define('DEFAULT_ACTION_NAME', 'index');
...
function urlRoute(){
	...
	$app_name = empty($route_arr[0]) ? DEFAULT_APP_NAME : strtolower($route_arr[0]);
	$controller_name = empty($route_arr[1]) ? DEFAULT_CONTROLLER_NAME : strtolower($route_arr[1]);
	$action_name = empty($route_arr[2]) ? DEFAULT_ACTION_NAME : strtolower($route_arr[2]);
...
function autoload($className){
	$array = array(
					BASE_PATH . 'apps/' . APP_NAME . '/model/' . $className . '.php',
					BASE_PATH . 'apps/' . APP_NAME . '/controller/' . $className . '.php',				
...
function model($model){
	...
	if( !is_object($objArray[$className]) ){
		if( !class_exists($className) ) {
			throw new Exception(APP_NAME. '/' . $className . '.php 模型类不存在');
		}
